...
def test_instance(objects, instance_name, n, c, z, tiempo):
	...
	if knapsack.get_value()!=z or len(missing_elements)!=0 or len(wrong_elements)!=0:
	...
	borrar1=0
	borrar2=0
	for i in knapsack.get_elements():
		borrar1+=i.get_value()
		borrar2+=i.get_weigth()
	print "valor z:"+str(z)+" valor obtenido:"+str(knapsack.get_value())+" borrar ####:#"+str(borrar1)+"lsjflsk: "+str(borrar2)
	...
	if len(wrong_elements)!=0:
		...
		for j in wrong_elements:
			print "----"+str(j.get_key())
	print "#######################"
	for x in knapsack.get_elements(): print str(x.get_key())+"-----"+str(x.get_value())+"----------"+str(x.get_x())
	print "\n"*2
